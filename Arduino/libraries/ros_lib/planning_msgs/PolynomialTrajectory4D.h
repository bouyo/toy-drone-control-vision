#ifndef _ROS_planning_msgs_PolynomialTrajectory4D_h
#define _ROS_planning_msgs_PolynomialTrajectory4D_h
...
namespace planning_msgs
{
  ...
  class PolynomialTrajectory4D : public ros::Msg
  {
    public:
      ...
      typedef planning_msgs::PolynomialSegment4D _segments_type;
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      if(segments_lengthT > segments_length)
        this->segments = (planning_msgs::PolynomialSegment4D*)realloc(this->segments, segments_lengthT * sizeof(planning_msgs::PolynomialSegment4D));
      segments_length = segments_lengthT;
      for( uint32_t i = 0; i < segments_length; i++){
      offset += this->st_segments.deserialize(inbuffer + offset);
        memcpy( &(this->segments[i]), &(this->st_segments), sizeof(planning_msgs::PolynomialSegment4D));
      }
     return offset;
    }

    const char * getType(){ return "planning_msgs/PolynomialTrajectory4D"; };
